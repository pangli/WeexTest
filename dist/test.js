...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/test.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-eedd92e0"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
// import pickItemBox from '@/views/components/pickItemBox';
// import pickItemBox from '@/views/components/pickDateBox'; 
...
exports.default = {
  ...
  components: {
    ...
    pickItemBox: _pickTimeBox2.default
  },
  data: function data() {
    return {
      ...
      url: '',
      params: '',
      items: ["HT-021-HQ-03-031", "HT-021-HQ-03-055", "HT-021-HQ-05-010", "HT-021-HQ-01-100", "全部11111112"],
      pageList: [{
        'name': '打电话',
        'pathName': ''
      }, {
      ...
      width: 750,
      height: 200,
      html: "<span style=\"color: rgb(204, 0, 0); font-family: arial; font-size: 13px; background-color: rgb(255, 255, 255);\">城市乐跑<\/span><span style=\"color: rgb(51, 51, 51); font-family: arial; font-size: 13px; background-color: rgb(255, 255, 255);\">赛是由万科主办,联合城市有影响力的企事业单位参与,以<\/span><span style=\"font-size: 24px;\"><strong><span style=\"font-family: arial; background-color: rgb(255, 255, 255); color: rgb(255, 0, 0);\">快乐、健康、友谊<\/span><\/strong><\/span><span style=\"color: rgb(51, 51, 51); font-family: arial; font-size: 13px; background-color: rgb(255, 255, 255);\">为宗旨的非商业性、非竞技性群众体育活动。< img src=\"group1\/M00\/00\/00\/CgCKXVrzqcKAUCavAAAU5Mscx_U1034063\" title=\"87.png\" alt=\"87.png\"\/><\/span><\/p>"
      //html: '<div style="background-color:red"><p style="color:#fff;font-size:40px;text-align:center;">打开相册</p><img src="https://gw.alicdn.com/tfs/TB1yopEdgoQMeJjy1XaXXcSsFXa-640-302.png" style=""/><p style="color:#fff;font-size:40px;text-align:center;">打开相册</p></div>'
    };
  },
  created: function created() {
    // this.postResult = JSON.stringify(weex.config)
  },
  ...
  methods: {
    ...
    goPage: function goPage(index) {
      ...
      // this.loadindexInfo();return;
      if (index == 0) {
        var modal = weex.requireModule('modal');
        modal.confirm({
          message: 'Do you want to call 18938952352?',
          okTitle: '确定',
          cancelTitle: '取消',
          duration: 0.3
        }, function (value) {
          console.log('confirm callback', value);
          general.call("18938952352");
        });
      } else if (index == 1) {
        var maxCount = 9;
        general.openImagePicker(maxCount, function (src) {
          _this.postResult = src;
          var obj = src; //JSON.parse(src);
          _this.src = obj.photos[0];
          _this.postResult = _this.src;
          imagesArr.push(_this.src);
        });
      } else if (index == 2) {
        general.takePicture(function (src) {
          _this.postResult = src;
          var obj = src; //JSON.parse(src);
          _this.src = obj.photos[0];
          _this.postResult = _this.src;
        ...
        var params = { "options": ["HT-021-HQ-03-031", "HT-021-HQ-03-055", "HT-021-HQ-05-010", "HT-021-HQ-01-100", "全部"] };
        normalPicker.openDialog(params, function (src) {
          _this.postResult = src;
        });
      } else if (index == 4) {
        var params = { "year": 0, "month": 1, "day": 1, "hours": 1, "minute": 1, "second": 0, "yearUnit": "年", "monthUnit": "月", "dayUnit": "日", "hoursUnit": "时", "minuteUnit": "分", "beginYear": 1970, "beginMonth": 0, "beginDay": 1, "endYear": 2018, "endMonth": 10, "endDay": 30 };
        ...
        var showIndex = 1;
        var images = ['http://pic.paopaoche.net/up/2015-7/201572710210.jpg', 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1527239918247&di=c6e01cc1148fb30ed42fda7aba8b7c70&imgtype=0&src=http%3A%2F%2Fimgsrc.baidu.com%2Fimage%2Fc0%253Dpixel_huitu%252C0%252C0%252C294%252C40%2Fsign%3D680d50449958d109d0eea1f2b820a9d3%2Fd000baa1cd11728bf249bf18c3fcc3cec3fd2ca7.jpg', 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1527239918233&di=c48d41349eb6f52cd852d9ee88a1ca31&imgtype=0&src=http%3A%2F%2Fi1.f.itc.cn%2Fupload%2Fsh%2F10444%2Fa_104431993.jpg'];
        images = images.concat(imagesArr);
        imagePreview.preview(images, showIndex);
      }
    },
    loadindexInfo: function loadindexInfo() {
      var _body;

      var server = 'http://ibptest.vanke.com/servers.api';
      // var server = 'http://192.168.191.1:8000/servers.api';
      var POST_URL = server + '/usersApi/userTenant'; //租户认证
      // var POST_URL = server+'/usersApi/getUserTenantByUserId'; //租户认证信息
      // var POST_URL = server+'/revenueApi/getRevenueList'; //营收填报list
      // var POST_URL = server+'/revenueApi/saveRevenue'; //新增营收填报
      // var POST_URL = server+'/homeApi/getParkInfo'; //招商信息
      // var POST_URL = server+'/homeApi/indexInfo'; //首页信息
      // var POST_URL = server+'/activeApi/getActiveListByMarketId'; //活动列表
      // var POST_URL = server+'/noticeApi/getNoticeListByMarketId';  //公告列表
      // var POST_URL = server+'/revenueApi/getRevenueList';  //营收填报列表
      // var POST_URL = server+'/revenueApi/saveRevenue';  //营收填报列表new
      var POST_URL = server + '/sendMsgApi/sendMsgCode'; //发送短信验证码
      var POST_URL = server + '/usersApi/userLogin'; //用户登录
      var self = this;
      var obj = {
        "head": { "version": "1.1.0", "clientOSType": 1, "accessToken": "57d52877043b9e819fda0736e46232f8", 'cityId': '021' },
        "body": (_body = {
          "marketId": "1111",
          "userId": "1111",
          // "revenueDate":"2018-5",
          ...
          // "fileImg":""
          "userName": "用户姓名",
          "email": "zhangjt06@vanke.com",
          "phone": "18938952352",
          "company": "公司名公司名",
          "department": "部门部门"
        }, _defineProperty(_body, 'account', "18617065718"), _defineProperty(_body, "password", "123456"), _body)
        //login account: 18617065718/123456
      ...
      stream.fetch({
        ...
        self.postResult = ret.body;
      }, function (response) {
        console.log('get in progress:' + response.length);
        // self.postResult = "bytes received:"+JSON.stringify(response);
      });
    },
    ...
    getDate: function getDate() {
      this.postResult = this.$refs.pickDateItem.value;
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["image"],
    attrs: {
      "src": "views/images/rz-success.png"
    },
    on: {
    ...
    staticClass: ["cell-arr"]
  }, [_vm._v(_vm._s(_vm.postResult))])])], 2)], 1)
},staticRenderFns: []}
