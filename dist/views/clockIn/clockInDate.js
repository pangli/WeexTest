...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/clockIn/clockInDate.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-61305694"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports = {
  ...
  "header": {
    "width": "750",
    "overflow": "hidden",
    "backgroundImage": "linear-gradient(145deg,rgb(0,209,255),rgb(0,190,255))"
  },
  "header-iOS7": {
    "paddingTop": "40",
    "overflow": "hidden"
...
var _billList = __webpack_require__(92);
...
var _base64ClockIn = __webpack_require__(60);
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      isiPhoneX: false,
      backIcon: (0, _base64ClockIn.backIcon)(),
      clockInStatusNei: (0, _base64ClockIn.clockInStatusNei)(),
      clockInStatusWai: (0, _base64ClockIn.clockInStatusWai)(),
      weeksArray: ['日', '一', '二', '三', '四', '五', '六'],
      days: [],
  ...
  mounted: function mounted() {
    var self = this;
    this.calculateEmptyGrids(self.curYear, self.curMonth);
    self.calculateDays(self.curYear, self.curMonth);
  },
  ...
  methods: {
    ...
    selectDay: function selectDay(dayNum) {
      // 过滤数据
      var self = this;
      self.curDay = dayNum;

      var currentDayDataList = [];
      var len = self.clockInHostory.length;
      for (var i = 0; i < len; i++) {
        if (self.clockInHostory[i].day == dayNum) {
          currentDayDataList.push(self.clockInHostory[i]);
        }
      }
      self.currentDayDataList = currentDayDataList;
      var t = self.curYear + "-" + (self.curMonth > 10 ? self.curMonth : '0' + self.curMonth) + '-' + dayNum;
      this.getClockInDataCurrent(t);
    },
    getClockInData: function getClockInData() {
      var self = this;
      _toast2.default.showLoadingMessage("正在获取签到数据…");
      var month = self.curMonth > 10 ? self.curMonth : '0' + self.curMonth;
      var params = {
        "signTime": '' + self.curYear + "-" + month
      };
      _api2.default.clockInHistoryList(params, function (ret) {
        _toast2.default.close();
        if (!ret.res) {
        ...
        if (ret.res.code == '10000' && ret.body) {
          // self.clockInHostory = ret.body.tenantSignList;
          self.totalClockIn = ret.body.total;

          var arrayL = [];
          var len = ret.body.tenantSignList.length;
          for (var i = 0; i < len; i++) {
            arrayL.push({
              signAddress: ret.body.tenantSignList[i].signAddress,
              signTime: '' + ret.body.tenantSignList[i].signTime.split(" ")[1].split(':')[0] + ':' + ret.body.tenantSignList[i].signTime.split(" ")[1].split(':')[1],
              day: ret.body.tenantSignList[i].signTime.split(" ")[0].split("-")[2]
            });
            self.clockDayArray.push(ret.body.tenantSignList[i].signTime.split(" ")[0].split("-")[2]);
          }
          self.clockInHostory = arrayL;
          // 今天
          self.selectDay(self.curDay);
    ...
    calculateEmptyGrids: function calculateEmptyGrids(year, month) {
      ...
      if (firstDayOfWeek > 0) {
        ...
        self.hasEmptyGrid = true, self.empytGrids = empytGrids;
      } else {
        self.hasEmptyGrid = false, self.empytGrids = [];
      }
    },
    ...
    // 遍历day的签到数量
    clockCount: function clockCount() {
      var self = this;
      var daysNew = [];
      var len = self.days.length;
      for (var i = 0; i <= len; i++) {
        var count = self.chongfuCount(self.days[i].day);
        if (count != 0) {}
        daysNew.push({
          day: i,
          count: count
    ...
    // 判断数组中重复值得个数
    chongfuCount: function chongfuCount(day) {
      var self = this;

      var arr = [];
      var count = 0;
      var result = self.clockInHostory;
      result.sort();
      for (var i = 0; i < result.length;) {
        ...
        arr.push({
          dayName: result[i].day,
          count: _count
        });
        i += _count;
      }
      self.clockIndayCount = arr.length;
      for (var k = 0; k < arr.length; k++) {
        if (arr[k].dayName == day) {
          count = arr[k].count;
        }
      }
    ...
    getClockInDataCurrent: function getClockInDataCurrent(t) {
      var self = this;
      var month = self.curMonth > 10 ? self.curMonth : '0' + self.curMonth;
      var params = {
        "signTime": t
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    class: ['header', _vm.isiOS7 ? 'header-iOS7' : '', _vm.isiPhoneX ? 'iPhoneX' : '']
  }, [_c('div', {
    staticClass: ["titlle"]
  }, [_c('div', {
    staticClass: ["ttl-left"],
    on: {
      "click": _vm.goBack
    }
  }, [_c('image', {
  ...
  }, [_vm._v("签到记录")]), _vm._m(0)]), _c('div', {
    staticClass: ["dateTitle"]
  }, [_c('div', {
    staticClass: ["dateTitle-left"],
    on: {
    ...
    staticClass: ["select-month-icon", "select-month-icon1"],
    attrs: {
      "src": _vm.backIcon
    }
  })]), _c('text', {
    staticClass: ["dateTitle-text"]
  }, [_vm._v(_vm._s(_vm.curYear) + "年" + _vm._s(_vm.curMonth) + "月（共签到" + _vm._s(_vm.clockIndayCount) + "次）")]), _c('div', {
    staticClass: ["dateTitle-right"],
    on: {
      "click": function($event) {
        _vm.nextMonth()
      }
    ...
    staticClass: ["select-month-icon", "select-month-icon2"],
    attrs: {
      "src": _vm.backIcon
    }
  })])]), _c('div', {
    staticClass: ["dateTop-tab"]
  }, _vm._l((_vm.weeksArray), function(item, index) {
    return _c('div', {
      staticClass: ["dateTop-tab-item"]
    }, [_c('text', {
      staticClass: ["dateTop-tab-item-text"]
    }, [_vm._v(_vm._s(item))])])
  })), _c('div', {
    staticClass: ["dateForm"]
  }, [_vm._l((_vm.empytGrids), function(item, index) {
    return (_vm.hasEmptyGrid) ? _c('div', {
      staticClass: ["dateForm-item-wrap"]
    }, [_c('div', {
      class: ['dateForm-item', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock' : '']
    }, [_c('div', {
      class: ['dateForm-item-nei', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock-nei' : '']
    }, [_c('text', {
      class: ['dateForm-text', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock-nei-text' : '']
    })])])]) : _vm._e()
    ...
    return _c('div', {
      ...
      on: {
        "click": function($event) {
          _vm.selectDay(item.day)
        }
      }
    }, [_c('div', {
      class: ['dateForm-item', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock' : '', item.count > 2 ? 'hasClockEnd' : '']
    }, [_c('div', {
      class: ['dateForm-item-nei', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock-nei' : '', , _vm.curDay == item.day ? 'curDay' : '']
    }, [_c('text', {
      class: ['dateForm-text', _vm.clockDayArray.indexOf('' + item.day) != -1 ? 'hasClock-nei-text' : '', _vm.curDay == item.day ? 'curDay-text' : '']
    }, [_vm._v(_vm._s(item.day))])])])])
  })], 2)]), _c('div', {
    staticClass: ["list-title"]
  }, [_c('text', {
    staticClass: ["list-title-text", "text1"]
  }, [_vm._v(_vm._s(_vm.curMonth) + "月" + _vm._s(_vm.curDay) + "号")]), _c('text', {
    staticClass: ["list-title-text", "text2"]
  }, [_vm._v("工作时常：")]), _c('text', {
    staticClass: ["list-title-text", "text3"]
  }, [_vm._v(_vm._s(_vm.currentDayData.tWorkTime))])]), _c('scroller', {
    staticClass: ["list"]
  }, [_c('div', {
    staticClass: ["item-wrap"]
  }, _vm._l((_vm.currentDayDataList), function(item, index) {
    return _c('div', {
      staticClass: ["lite-item"]
    }, [_c('div', {
      staticClass: ["item"]
    }, [_c('div', {
      ...
      attrs: {
        "src": _vm.clockInStatusNei
      }
    }), _c('text', {
      staticClass: ["item-top-right"]
    }, [_vm._v(_vm._s(item.signTime))])]), _c('div', {
      staticClass: ["item-bottom"]
    }, [_c('text', {
      staticClass: ["item-bottom-text"]
    }, [_vm._v(_vm._s(item.signAddress))])])])])
  }))])])
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["ttl-right"]
  }, [_c('text', {
    staticClass: ["ttl-right-text"]
  })])
}]}
module.exports.render._withStripped = true
...
// 定位
var dinwei = function dinwei() {
   return 'data:image/png;base64,';
};
var clockIn = function clockIn() {
   return 'data:image/png;base64,';
};

// 打卡记录
var clockInJilu = function clockInJilu() {
   return 'data:image/png;base64,';
};

//签到状态 内勤
var clockInStatusNei = function clockInStatusNei() {
   return 'data:image/png;base64,';
};
// 签到状态 外勤
var clockInStatusWai = function clockInStatusWai() {
   return 'data:image/png;base64,';
};
// 地图上的定位
var adressSelect = function adressSelect() {
   return 'data:image/png;base64,';
};
...
module.exports = {
   ...
   dinwei: dinwei,
   clockIn: clockIn,
   clockInJilu: clockInJilu,
   clockInStatusNei: clockInStatusNei,
   clockInStatusWai: clockInStatusWai,
