...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/clockIn/selectAdress.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3da53d4d"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var localSearch = weex.requireModule('location');
var localSearchT = weex.requireModule('localSearch');
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      isiPhoneX: false,
      back: (0, _base64ClockIn.back)(),
      adressSelect: (0, _base64ClockIn.adressSelect)(),
      dinwei: (0, _base64ClockIn.dinwei)(),
      longitude: "",
      latitude: "",
      adressList: [],
      currentAddress: "",
  ...
  mounted: function mounted() {
    _toast2.default.showLoadingMessage("正在获取附近地址…");
  },
  ...
  methods: {
    ...
    sure: function sure(a, b) {
      var self = this;
      _toast2.default.close();

      var url = _global2.default.getUrl(weex.config.bundleUrl, encodeURI(encodeURI('views/clockIn/clockInTips.js' + '?currentAddress=' + a + '&currentAddressName=' + b + '&longitude=' + self.longitude + '&latitude=' + self.latitude)));
      navigator.push({
        url: url,
        animated: 'true'
      });
    },
    selectEd: function selectEd(index, addresName, address) {
      this.showAddress = index;
      this.currentAddressName = addresName;
      this.currentAddress = address;
    },
    getNearAddress: function getNearAddress(longitude, latitude) {
      var self = this;
      var param = { "longitude": longitude, "latitude": latitude };
      // 附近的地点
      localSearchT.startLocalSearch(param, function (res) {
        if (res.res == '1') {
          _toast2.default.close();
          self.adressList = res.data;
          self.currentAddress = res.data[0].address;
          self.currentAddressName = res.data[0].name;
        } else {
          _toast2.default.close();
          _toast2.default.showMessage('获取位置失败');
        }
      });
    },
    viewstart: function viewstart() {//地图开始加载事件
    },
    viewfinish: function viewfinish() {
      // 地图加载完成事件 
      var self = this;
      // 定位
      localSearch.startLocation(function (res) {
        if (res.res == "1") {
          self.longitude = res.data.longitude;
          self.latitude = res.data.latitude;

          self.getNearAddress(res.data.longitude, res.data.latitude);
        } else {
          _toast2.default.showMessage('获取位置失败');
        }
      });
    },
    viewerror: function viewerror() {//地图加载失败事件 
    },
    locationfail: function locationfail() {//地图定位失败事件
    },
    locationfinish: function locationfinish() {//地图定位成功事件
    }
  }
};
  ...
  return _c('div', {
    ...
    on: {
      "click": function($event) {
        _vm.sure(_vm.currentAddress, _vm.currentAddressName)
      }
    }
  }, [_vm._v("确定")])])])]), _c('mapComponent', {
    ref: "mapComponent",
    staticClass: ["mapComponent"],
    attrs: {
    ...
    on: {
      ...
      "viewfinish": function($event) {
        _vm.viewfinish()
      },
      "viewerror": function($event) {
        _vm.viewerror()
      },
      "locationfail": function($event) {
        _vm.locationfail()
      },
      "locationfinish": function($event) {
        _vm.locationfinish()
      }
    }
  }, [_c('image', {
    ...
    attrs: {
      "src": _vm.adressSelect
    }
  })]), _c('scroller', {
    staticClass: ["adress-list", "scroller"]
  }, _vm._l((_vm.adressList), function(item, index) {
    return _c('div', {
      staticClass: ["adress-item"],
      on: {
        "click": function($event) {
          _vm.selectEd(index, item.name, item.address)
        }
      }
    }, [_c('text', {
      staticClass: ["adress-item-title"]
    }, [_vm._v(_vm._s(item.name))]), _c('text', {
      staticClass: ["adress-item-text"]
    }, [_vm._v(_vm._s(item.address))]), (index == _vm.showAddress) ? _c('image', {
      staticClass: ["selectAdressIcon"],
      attrs: {
        "src": _vm.dinwei
      }
    }) : _vm._e()])
