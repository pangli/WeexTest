...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/activity/activityForm.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-45f32904"
...
exports.default = {
  data: function data() {
    return {
      ...
      result: '',
      regDictFormList: '',
      order: weex.config.params,
      allHasVal: false,
      orderId: '123345566',

      weexParam: weex.config.params,
      testIndex: 222,
      actId: ''
    };
  },
  ...
  methods: {
    ...
    nextPage: function nextPage() {
      this.insertActiviteRegistration();
    },
    changeRadioSex: function changeRadioSex(order, param) {
      this.radioIndex = param;
      this.regDictFormList[order].regValue = String(param);
    },

    getRegDictFromByActId: function getRegDictFromByActId() {
      var _this3 = this;
      ...
      _api2.default.getRegDictFromByActId(params, function (ret) {
        if (!ret.res) {
          toast.showErrorMessage('服务异常');
          return;
        }
        if (ret.res.code == '10000' && ret.body) {
          if (ret.body.regDictFormList) {
            var regDictFormList = ret.body.regDictFormList;
            _this3.result = regDictFormList;
            if (regDictFormList.length === 1 && regDictFormList[0].regKey === 'dict_gender') {
              _this3.allHasVal = true;
            }
            var _this = _this3;
            for (var i = 0; i < regDictFormList.length; i++) {
              regDictFormList[i].regValue = "";
              regDictFormList[i].warnMessage = "";
              regDictFormList[i].isOk = false;

              if (regDictFormList[i].regKey === 'dict_gender') {
                regDictFormList[i].regValue = '0';
                regDictFormList[i].isOk = true;
              }
            }
            _this3.regDictFormList = regDictFormList;
          } else {
    ...
    insertActiviteRegistration: function insertActiviteRegistration() {
      var _this4 = this;

      toast.showLoadingMessage("报名中...");
      var param = _global2.default.getParams(weex.config.bundleUrl);
      var params = {
        'actId': this.actId,
        ...
        "marketId": weex.config.params.marketId,
        "regList": this.regDictFormList
      };

      _api2.default.insertActiviteRegistration(params, function (ret) {
        console.log(ret);
        toast.close();
        if (!ret.res) {
        ...
        if (ret.res.code == '10000' && ret.body) {
          if (ret.body.orderId) {
            ...
            if (ret.body.type == 1) {
              var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/activity/activityResult.js' + '?actId=' + _this4.actId + '&orderId=' + _this4.orderId + '&listgo=2');
              navigator.push({
                url: url,
                animated: 'true'
              });
    ...
    pickDate: function pickDate(i) {
      ...
      // var params = {"year":1,"month":1,"day":1,"hours":0,"minute":0,"second":0,"yearUnit":"","monthUnit":"月","dayUnit":"日","hoursUnit":"时","minuteUnit":"分","beginYear":1970,"beginMonth":0,"beginDay":1,"endYear":endYear,"endMonth":endMonth,"endDay":endDay};
      var params = {
        "year": 1,
        "month": 1,
        "day": 1,
      ...
      timePicker.openTimePicker(params, function (src) {
        // this.dateIndex = src;
        _this5.regDictFormList[i].regValue = src;

        if (currentDate >= new Date(src)) {
          _this5.regDictFormList[i].warnMessage = '';
          _this5.regDictFormList[i].isOk = true;
        } else {
          _this5.regDictFormList[i].warnMessage = '所选日期不能大于当前日期';
          _this5.regDictFormList[i].isOk = false;
        }

        for (var j = 0; j < _this5.regDictFormList.length; j++) {
          if (_this5.regDictFormList[j].isOk) {
            _this5.allHasVal = true;
          } else {
            _this5.allHasVal = false;
            break;
          }
        }
      });
    ...
    changeVal: function changeVal(event) {
      this.testIndex = event.target.attr;

      var order = parseInt(event.target.attr.order - 1);
      var myreg = eval(event.target.attr.myreg);

      if (myreg.test(event.value) && event.value.trim().length) {
        this.regDictFormList[order].warnMessage = '';
        this.regDictFormList[order].isOk = true;

        for (var i = 0; i < this.regDictFormList.length; i++) {
          if (this.regDictFormList[i].isOk) {
            this.allHasVal = true;
          } else {
            this.allHasVal = false;
            break;
          }
        }
      } else {
        this.regDictFormList[order].warnMessage = '格式不正确请重新输入';
      }
    }
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, _vm._l((_vm.regDictFormList), function(item, order) {
    return _c('div', [_c('div', {
    ...
    }, [_vm._v(_vm._s(item.regName))]), _c('text', {
      staticClass: ["cell_sign"]
    }, [_vm._v("*")])]), (item.regType == 1) ? _c('div', {
      staticClass: ["butn_box"]
    }, [_c('input', {
      staticClass: ["butn_name"],
      attrs: {
        "type": "text",
        "myreg": item.regularName,
        "order": item.sortOrder,
        "placeholder": '请输入' + item.regName,
        "value": (item.regValue)
      },
      on: {
        "change": _vm.changeVal,
        "input": function($event) {
          _vm.$set(item, "regValue", $event.target.attr.value)
        }
      }
    }), (item.warnMessage) ? _c('text', {
      staticClass: ["warnMessage"]
    }, [_vm._v(_vm._s(item.regName) + _vm._s(item.warnMessage))]) : _vm._e()]) : _vm._e(), (item.regType == 2) ? _c('div', {
      staticClass: ["butn_box_row"]
    }, [_c('div', {
      staticClass: ["butn_radio_box"]
    }, [_c('text', {
      staticClass: ["butn_radio"],
      on: {
        "click": function($event) {
          _vm.changeRadioSex(order, 0)
        }
      }
    }, [_vm._v("男")]), (_vm.radioIndex == 0) ? _c('image', {
      staticClass: ["butn_radio_img"],
      staticStyle: {
        width: "164px",
        height: "64px"
      },
      attrs: {
        "src": "data:image/png;base64,"
      }
    }) : _vm._e()]), _c('div', {
      staticClass: ["butn_radio_box"]
    }, [_c('text', {
      ...
      on: {
        "click": function($event) {
          _vm.changeRadioSex(order, 1)
        }
      }
    }, [_vm._v("女")]), (_vm.radioIndex == 1) ? _c('image', {
      staticClass: ["butn_radio_img"],
      staticStyle: {
        width: "164px",
        height: "64px"
    ...
    }) : _vm._e()])]) : _vm._e(), (item.regType == 3) ? _c('div', {
      staticClass: ["butn_box"]
    }, [_c('text', {
      staticClass: ["butn_name"],
      attrs: {
    ...
    }, [_vm._v(_vm._s(item.regValue))]), (item.warnMessage) ? _c('text', {
      staticClass: ["warnMessage"]
    }, [_vm._v(_vm._s(item.warnMessage))]) : _vm._e()]) : _vm._e()])])
  })), _c('div', {
    staticClass: ["bottom_box"]
  }, [(!_vm.allHasVal) ? _c('text', {
    staticClass: ["bottom_butn", "bottom_butn_gray"]
  }, [_vm._v("下一步")]) : _vm._e(), (_vm.allHasVal) ? _c('text', {
    staticClass: ["bottom_butn"],
    on: {
      "click": _vm.nextPage
    }
  }, [_vm._v("下一步")]) : _vm._e()])], 1)
