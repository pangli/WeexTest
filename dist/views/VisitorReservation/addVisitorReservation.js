...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/VisitorReservation/addVisitorReservation.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-7960dacd"
...
module.exports = {
  ...
  "form": {
    ...
    "boxShadow": "0px 8px 16px 0px rgba(0, 0, 0, 0.02)",
    "borderRadius": "16"
  },
  "bottomButton": {
    "width": "750",
...
var normalPicker = weex.requireModule('normalPicker');
...
exports.default = {
  ...
  created: function created() {
    ...
    var Stack = new BroadcastChannel('Avengers');
    Stack.onmessage = function (event) {
      _this.userInfo = event.data;
      var regList = _this.regList;
      for (var index in regList) {
        if (regList[index].regKey === 'examine_user_name') {
          regList[index].regValue = event.data.userName;
        }
        if (regList[index].regKey === 'visitor_company' && event.data.company && event.data.company !== '') {
          _this.companyName = event.data.company;
          regList[index].regValue = event.data.company;
        }
        if (regList[index].regKey === 'visitor_floor' && event.data.company && event.data.company !== '') {
          regList[index].regValue = event.data.address;
        }
      }
      _this.regList = regList;
    };
    var Stave = new BroadcastChannel('getCompanyList');
    Stave.onmessage = function (event) {
      _this.companyInfo = event.data;
      var regList = _this.regList;
      for (var index in regList) {
        if (regList[index].regKey === 'visitor_company') {
          _this.companyName = event.data.companyName;
          regList[index].regValue = event.data.companyName;
        }
        if (regList[index].regKey === 'visitor_floor') {
          regList[index].regValue = event.data.address;
        }
      }
      _this.regList = regList;
    };
    this.getVisitorRegByParams();
  },
  ...
  methods: {
    ...
    searchPerson: function searchPerson() {
      var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/VisitorReservation/searchPerson.js');
      this.currentUrl = url;
      navigator.push({
        url: url,
        animated: 'true'
    ...
    getVisitorRegByParams: function getVisitorRegByParams() {
      var _this2 = this;

      _api2.default.getVisitorRegByParams({
        "marketId": weexParams.marketId
      }, function (res) {
        _this2.regList = res.body.regList;
        _this2.settingId = res.body.setting.id;
        var regList = res.body.regList,
            idTypeList = res.body.idTypeList;
        for (var key in regList) {
          if (regList[key].regKey === 'examine_user_name') {
            _this2.hasPerson = true;
          }
          if (regList[key].regKey === 'visitor_user_name') {
            regList[key].regValue = weexParams.userTrueName;
          }
          if (regList[key].regKey === 'visitor_mobile') {
            regList[key].regValue = weexParams.phone;
          }
          if (regList[key].regKey === 'visitor_id_type') {
            regList[key].regValue = idTypeList[0].dictName;
          }
        }
        _this2.regList = regList;
        if (res.body.dictList) {
          var dictList = res.body.dictList;
          for (var _key in dictList) {
            _this2.dictList.push(dictList[_key].dictName);
          }
        }
        if (res.body.idTypeList) {
          for (var _key2 in idTypeList) {
            _this2.idTypeList.push(idTypeList[_key2].dictName);
          }
          _this2.Idtype = idTypeList[0].dictName;
        }
      });
    },
    pickTime: function pickTime() {
      ...
      var params = this.setProps;
      var PropertyRepairMinutes = myDate.getMinutes() + 1;
      var PropertyRepairHours = myDate.getHours();
      var PropertyRepairDay = myDate.getDate();
      var PropertyRepairMonth = myDate.getMonth() + 1;
      var PropertyRepairYear = myDate.getFullYear();
      if (PropertyRepairMinutes <= 30) {
        PropertyRepairMinutes = 30;
      } else {
        PropertyRepairMinutes = 0;
        PropertyRepairHours = PropertyRepairHours + 1;
      }
      params.beginYear = _global2.default.formatNumber(PropertyRepairYear);
      params.endYear = _global2.default.formatNumber(PropertyRepairYear + 1);
      params.beginMonth = _global2.default.formatNumber(PropertyRepairMonth);
      params.beginDay = _global2.default.formatNumber(PropertyRepairDay);
      params.beginHours = _global2.default.formatNumber(PropertyRepairHours);
      params.beginMinute = _global2.default.formatNumber(PropertyRepairMinutes);
      timePicker.openTimePicker(params, function (src) {
        _this3.date = src;
        var regList = _this3.regList;
        for (var index in regList) {
          if (regList[index].regKey === 'visitor_date') {
            regList[index].regValue = src;
          }
        }
        _this3.regList = regList;
        _this3.cango();
      });
    },
    pickItem: function pickItem() {
      ...
      var params = {};
      params.options = this.dictList;
      normalPicker.openDialog(params, function (src) {
        _this4.type = src;
        var regList = _this4.regList;
        for (var index in regList) {
          if (regList[index].regKey === 'visitor_type') {
            regList[index].regValue = src;
          }
        }
        _this4.regList = regList;
        _this4.cango();
      });
    },
    pickIdItem: function pickIdItem() {
      var _this5 = this;

      var params = {};
      params.options = this.idTypeList;
      normalPicker.openDialog(params, function (src) {
        _this5.Idtype = src;
        var regList = _this5.regList;
        for (var index in regList) {
          if (regList[index].regKey === 'visitor_id_type') {
            regList[index].regValue = src;
          }
        }
        _this5.regList = regList;
        _this5.cango();
      });
    },
    getValue: function getValue(e, key) {
      var regList = this.regList;
      for (var index in regList) {
        if (regList[index].regKey === key) {
          regList[index].regValue = e.value;
        }
      }
      this.regList = regList;
      this.cango();
    },
    getVisitorId: function getVisitorId(e) {
      var regList = this.regList;
      for (var index in regList) {
        if (regList[index].regKey === 'visitor_id') {
          regList[index].regValue = e.value;
        }
      }
      this.visitorId = e.value;
      this.regList = regList;
      this.cango();
    },
    searchCompanyName: function searchCompanyName() {
      if (!this.hasPerson) {
        var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/VisitorReservation/companyList.js');
        navigator.push({
          url: url,
          animated: 'true'
        });
    ...
    cango: function cango() {
      var regList = this.regList,
          goNext = true;
      for (var index in regList) {
        if (regList[index].regValue === '' || regList[index].regValue == 0) {
          goNext = false;
        }
      }
      this.goNext = goNext;
    },
    insertVisitorReg: function insertVisitorReg() {
      var _this6 = this;
      ...
      if (this.userInfo !== '') {
        params.examineInfo = this.userInfo;
      }
      if (this.companyInfo !== '') {
        params.complaintInfo = this.companyInfo;
      }
      var regList = this.regList,
          goNext = true,
          myreg = /^[1][3,4,5,7,8][0-9]{9}$/;
      for (var index in regList) {
        if (regList[index].regKey === 'visitor_mobile') {
          if (!myreg.test(regList[index].regValue)) {
            goNext = false;
          }
        }
      }
      if (goNext) {
        _api2.default.insertVisitorReg(params, function (res) {
          if (res.res.code === 10000) {
            _this6.goBack();
          } else {
            toast.showErrorMessage(res.res.msg);
          }
        });
      } else {
        toast.showErrorMessage('请输入正确手机号');
      }
    }
  },
  filters: {
    place: function place(value) {
      return "请输入" + value;
    }
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["bg"],
    attrs: {
      "src": "data:image/png;base64,"
    }
  }), _c('div', {
    staticClass: ["formBox"]
  }, [_c('image', {
    staticClass: ["vanke"],
    attrs: {
      "src": "data:image/png;base64,"
    }
  }), _c('text', {
    staticClass: ["welcome"]
  }, [_vm._v("欢迎您来访万科")]), _c('div', {
    staticClass: ["form"]
  }, _vm._l((_vm.regList), function(item) {
    return _c('div', [(item.regKey === 'examine_user_name') ? _c('div', {
      staticClass: ["nameBox"],
      on: {
        "click": _vm.searchPerson
      }
    }, [_c('div', {
      staticClass: ["nameBoxLeft"]
    }, [_c('text', {
      staticClass: ["label"]
    }, [_vm._v(_vm._s(item.regName))])]), _c('div', {
      staticClass: ["nameBoxRight"]
    }, [(!_vm.userInfo) ? _c('text', {
      staticClass: ["placeholder"]
    }, [_vm._v("请选择")]) : _c('text', {
      staticClass: ["value"]
    }, [_vm._v(_vm._s(_vm.userInfo.userName))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_date') ? _c('div', {
      staticClass: ["nameBox"],
      on: {
        "click": _vm.pickTime
      }
      ...
      staticClass: ["value"]
    }, [_vm._v(_vm._s(_vm.date))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_company' && (!_vm.hasPerson || _vm.companyName !== '')) ? _c('div', {
      staticClass: ["nameBox"],
      on: {
        "click": _vm.searchCompanyName
      }
    }, [_c('div', {
      staticClass: ["nameBoxLeft"]
    }, [_c('text', {
      ...
      staticClass: ["nameBoxRight"]
    }, [(_vm.companyName === '') ? _c('text', {
      staticClass: ["placeholder"]
    }, [_vm._v("请选择")]) : _c('text', {
      staticClass: ["value"]
    }, [_vm._v(_vm._s(_vm.companyName))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_floor' && (!_vm.hasPerson || item.regValue != 0)) ? _c('div', {
      staticClass: ["nameBox"]
    }, [_c('div', {
      staticClass: ["nameBoxLeft"]
    }, [_c('text', {
      ...
      staticClass: ["nameBoxRight"]
    }, [(item.regValue == 0) ? _c('text', {
      staticClass: ["placeholder"]
    }, [_vm._v("请选择" + _vm._s(item.regName))]) : _c('text', {
      staticClass: ["value"]
    }, [_vm._v(_vm._s(item.regValue))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_type') ? _c('div', {
      staticClass: ["nameBox"],
      on: {
        "click": _vm.pickItem
      }
      ...
      staticClass: ["value"]
    }, [_vm._v(_vm._s(_vm.type))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_id_type') ? _c('div', {
      staticClass: ["nameBox"]
    }, [_c('div', {
      staticClass: ["nameBoxLeft", "borderRight"],
      on: {
        "click": _vm.pickIdItem
      }
    }, [_c('text', {
      staticClass: ["label"]
    }, [_vm._v(_vm._s(_vm.Idtype))]), _c('image', {
      staticClass: ["yjt"],
      attrs: {
        "src": _vm.yjt
      }
      ...
      on: {
        "change": function($event) {
          _vm.getVisitorId($event)
        }
      }
    })])]) : _vm._e(), (item.regKey === 'visitor_mobile') ? _c('div', {
      staticClass: ["nameBox"]
    }, [_c('div', {
      staticClass: ["nameBoxLeft"]
    }, [_c('text', {
      ...
      attrs: {
        "type": "number",
        "placeholder": _vm._f("place")(item.regName),
        "value": item.regValue == 0 ? '' : item.regValue
      },
      on: {
        "change": function($event) {
          _vm.getValue($event, item.regKey)
        }
      }
    })])]) : _vm._e(), (item.regKey !== 'examine_user_name' && item.regKey !== 'visitor_floor' && item.regKey !== 'visitor_mobile' && item.regKey !== 'visitor_date' && item.regKey !== 'visitor_type' && item.regKey !== 'visitor_id_type' && item.regKey !== 'visitor_company' && item.regKey !== 'visitor_id') ? _c('div', {
      staticClass: ["nameBox"]
    }, [_c('div', {
      staticClass: ["nameBoxLeft"]
    }, [_c('text', {
