...
/* styles */
__vue_styles__.push(__webpack_require__(659)
)
...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/revenueFill/update.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-fa8e6d50"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  created: function created() {
    ...
    if (_global2.default.getParams(weex.config.bundleUrl).jine) {
      this.getRevenueById();
    }
  },
  ...
  methods: {
    ...
    saveRevenue: function saveRevenue() {
      toast.close();
      var date = _global2.default.getParams(weex.config.bundleUrl).date;
      date = date.split("-").join("");
      toast.showLoadingMessage("提交中...");
      _api2.default.saveRevenue({
        "userId": weexParams.userId,
        "userName": weexParams.userName,
        "revenueDate": date,
        "revenueAmount": this.showe,
        "storeId": _global2.default.getParams(weex.config.bundleUrl).value,
    ...
    getRevenueById: function getRevenueById() {
      var _this3 = this;

      toast.showLoadingMessage("获取详情中...");
      var date = _global2.default.getParams(weex.config.bundleUrl).date;
      date = date.split("-").join("");
      _api2.default.getRevenueById({
        "userId": weexParams.userId,
        "revenueDate": date,
        "revenueAmount": _global2.default.getParams(weex.config.bundleUrl).jine,
        "storeId": _global2.default.getParams(weex.config.bundleUrl).value
      }, function (ret) {
        toast.close();
        _this3.$refs.showEValue.value = _global2.default.getParams(weex.config.bundleUrl).jine;
        _this3.showe = _global2.default.getParams(weex.config.bundleUrl).jine;
        _this3.back = ret;
        if (ret.res.code === 10000) {
          var images = ret.body.imageList;
          var imageList = [];
          for (var i = 0; i < images.length; i++) {
            imageList.push(images[i].imgPath);
          }
          _this3.photos = imageList;
          _this3.count = imageList.length;
          _this3.updatePic = imageList.join(',');
        }
      });
    }
  }
