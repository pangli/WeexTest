...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/address/addAddress.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-74b83acd"
...
module.exports = {
  ...
  "pretermissionIcon": {
    ...
    "boxShadow": "0px 4px 8px 0px rgba(231, 231, 231, 0.5)",
    "borderRadius": 100
  },
  "pretermissionActiveBox": {
    "width": "112",
    "height": "56",
    "borderRadius": "28",
    "borderTopWidth": "4",
...
exports.default = {
  ...
  created: function created() {
    ...
    this.phone = weexParams.phone;
    if (_global2.default.getParams(weex.config.bundleUrl).address != undefined) {
      if (_global2.default.getParams(weex.config.bundleUrl).address === 'new') {
        this.addressId = _global2.default.getParams(weex.config.bundleUrl).address;
        this.pretermission = true;
      } else {
        this.address = JSON.parse(decodeURI(_global2.default.getParams(weex.config.bundleUrl).address));
        this.addressId = this.address.id;
        this.userName = this.address.sendeeName;
        this.phone = this.address.sendeeTel;
        this.detail = this.address.sendeeAddress;
        if (this.address.status == '0') {
          this.pretermission = true;
        } else {
  ...
  methods: {
    ...
    check: function check() {
      if (this.userName == '' || this.phone == '' || this.detail == '') {
        this.goNext = false;
      } else {
        this.goNext = true;
      }
    },
    addMbAddress: function addMbAddress() {
      var addressId = '',
          status = '1';
      ...
      if (this.pretermission) {
        status = '0';
      } else {
        status = '1';
      }
      _api2.default.addMbAddress({
        "addressId": addressId,
        "memberCode": weexParams.memberCode,
        "sendeeName": this.userName,
        "sendeeTel": this.phone,
        "sendeeAddress": this.detail,
        "status": status //0是默认,1是正常,9是删除
      }, function (res) {
        if (res.res.code === 10000) {
          _BroadcastChannel2.default.addressSend('refresh');
          _BroadcastChannel2.default.shoppingSend(res.body.mbAddress);
          navigator.pop();
        }
      });
    },
    ...
    changePretermission: function changePretermission() {
      if (this.addressId !== 'new') {
        this.pretermission = false;
      }
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["detailAddressBox"]
  }, [_c('text', {
    staticClass: ["addressLabel"]
  }, [_vm._v("详细地址")]), _c('textarea', {
    ...
    on: {
      "input": function($event) {
        _vm.detail = $event.target.attr.value
      }
    }
  })]), _c('div', {
    staticClass: ["inputBox"]
    ...
    staticClass: ["label"]
  }, [_vm._v("设置默认地址")]), (_vm.pretermission) ? _c('div', {
    staticClass: ["pretermissionActiveBox"],
    on: {
      "click": _vm.changePretermission
    }
  }, [_c('div', {
    staticClass: ["pretermissionActiveIcon"]
  })]) : _c('div', {
    staticClass: ["pretermissionBox"],
    on: {
      "click": function($event) {
        _vm.pretermission = true
      }
    }
  }, [_c('div', {
    staticClass: ["pretermissionIcon"]
  })])]), (_vm.addressId && _vm.addressId !== 'new') ? _c('text', {
    staticClass: ["delete"],
    on: {
      "click": _vm.deleteAddress
    }
  }, [_vm._v("删除收货地址")]) : _vm._e()]), _c('div', {
    staticClass: ["bottomButton"]
  }, [(_vm.goNext) ? _c('text', {
    staticClass: ["bottomButtonActive"],
    on: {
      "click": _vm.addMbAddress
    }
  }, [_vm._v("保存")]) : _c('text', {
    staticClass: ["cantGo"]
  }, [_vm._v("保存")])])], 1)
