...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/Feedback/form.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-f6077402"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      details: '',
      add: (0, _base.add)(),
      items: ['欣欣咖啡-概念馆', '欣欣咖啡-概念馆', '玖玖甜品8624虹桥店', '全部'],
      addTrue: false,
      photos: '',
      updatePic: '',
      count: 0,
      tenant: 0,
      pickItemPlaceHolder: '请选择商铺'
    };
  },
  ...
  methods: {
    ...
    goType: function goType() {
      var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/Feedback/type.js');
      this.currentUrl = url;
      navigator.push({
        url: url,
        animated: 'true'
    ...
    addMsg: function addMsg() {
      var that = this;
      _api2.default.addComplaint({
        "marketId": weexParams.marketId,
        "userId": weexParams.userId,
        "userName": that.lianxiRenValue,
        "phoneNum": that.tel,
        "content": that.details,
        "fileImg": this.updatePic,
        "isTenant": this.tenant,
        "tenant": this.value
      }, function (ret) {
        toast.close();
      ...
      if (this.photos.length === 0) {
        ...
        (0, _uploadService2.default)(params, function (ret) {
          ...
          if (ret.res && ret.returnJson && ret.returnJson.res && ret.returnJson.res.code == 10000 && ret.returnJson.body) {
            if (_this2.updatePic === '') {
              _this2.updatePic = ret.returnJson.body.imgId;
            } else {
              _this2.updatePic = _this2.updatePic + ',' + ret.returnJson.body.imgId;
            }
            _this2.count = _this2.count + 1;
            if (_this2.count >= _this2.photos.length) {
              _this2.addMsg();
            } else {
              _this2.uploadImage();
            }
    ...
    getShop: function getShop() {
      this.value = this.$refs.pick.value;
    },
    getCompanyIdByUserId: function getCompanyIdByUserId() {
      var _this3 = this;

      toast.showLoadingMessage("获取商铺中...");
      _api2.default.getCompanyIdByUserId({
        userId: weexParams.userId
      }, function (ret) {
        toast.close();
        console.log(ret);
        if (ret.res.code === 10000) {
          var items = ret.body.userTenantList;
          var item = [];
          for (var i = 0; i < items.length; i++) {
            item.push(items[i].company);
          }
          _this3.items = item;
          if (item.length >= 1) {
            _this3.value = item[0];
            _this3.pickItemPlaceHolder = item[0];
          } else {
            toast.showMessage("获取商铺失败！");
          }
        }
      });
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  ...
  })], 1), (_vm.tenant === 1) ? _c('pickItemBox', {
    ref: "pick",
    attrs: {
      "label": "我的企业",
      "items": _vm.items,
      "placeholder": _vm.pickItemPlaceHolder
    },
    on: {
      "changeValue": _vm.getShop
    }
  }) : _vm._e(), _c('div', {
    staticClass: ["textAreaBox"]
  }, [_c('text', {
    staticClass: ["title"]
  }, [_vm._v("意见反馈")]), _c('textarea', {
