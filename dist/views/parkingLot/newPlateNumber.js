...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/parkingLot/newPlateNumber.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-675b0eb4"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      inputText2: "请输入车牌号",
      carId: "",
      keyBoardClear: (0, _base64ParkingLot.keyBoardClear)(),
      getCarIdType: "11111",
      getCarIdTypeIndex: "22222",
      deleteKeyCountIndex: "33333",
      // 自定义键盘结束
      back: (0, _base64ParkingLot.back)(),
      rightIcon: (0, _base64ParkingLot.rightIcon)(),
      dateList: ["充值", "月租"],
      chooseItem: '请选择',
      chooseItemId: '',
      showKeyBoard: true,
  ...
  created: function created() {
    ...
    self.addType = param.type;
    if (self.addType == '0') {//新增
    } else {
      //编辑
      storage.getItem('editPlateNo', function (event) {
        if (event.data == 'undefined' || event.data == "") {} else {
          self.typeText = '编辑车牌';
          self.inputText2 = JSON.parse(event.data).plateNo;
          self.chooseItemId = JSON.parse(event.data).payType;
          self.chooseItem = self.dateList[JSON.parse(event.data).payType - 1];
          self.editItem = JSON.parse(event.data);
        }
      });
    }
  },
  ...
  methods: {
    ...
    // 选择数据
    selectItem: function selectItem(title) {
      var self = this;
      self.overlayClicked();
      picker.pick({
        items: self.dateList,
        ...
        if (res.result === 'success') {
          self.chooseItem = self.dateList[res.data];
          self.chooseItemId = res.data + 1;
        } else {}
      });
    },
    ...
    save: function save() {
      ...
      if (param.type == '0') {
        //新增
        this.bindCompanyPlateNo();
      } else {
        //编辑
        this.editPlateNo();
      }
    },
    bindCompanyPlateNo: function bindCompanyPlateNo() {
      var self = this;
      if (self.chooseItemId == "") {
        modal.alert({
          message: "请选择收费类型"
        });
        return;
      ...
      var param = _global2.default.getParams(weex.config.bundleUrl);
      var weexParams = weex.config.params || {};
      toast.showLoadingMessage("正在绑定...");
      var params = {
        "payType": self.chooseItemId,
        "userName": weexParams.userTrueName,
        "palteNo": self.inputText2,
        "parkCompanyId": param.parkCompanyId
      };
      _api2.default.bindCompanyPlateNo(params, function (ret) {
        toast.close();
        if (!ret.success) {
        ...
        if (ret.code == '10000' && ret.success) {
          toast.showMessage('绑定成功', 2);
          setTimeout(function () {
            var addSuccess = new BroadcastChannel("addSuccess");
            addSuccess.postMessage(encodeURI(encodeURI('1')));
            navigator.pop();
          }, 1500);
        } else {
    ...
    editPlateNo: function editPlateNo() {
      var self = this;
      if (self.chooseItemId == "") {
        modal.alert({
          message: "请选择收费类型"
      ...
      var param = _global2.default.getParams(weex.config.bundleUrl);
      var weexParams = weex.config.params || {};
      toast.showLoadingMessage("正在编辑...");
      var params = {
        "bindPlateNoId": self.editItem.id,
        "payType": self.chooseItemId,
        "userName": weexParams.userTrueName,
        "palteNo": self.inputText2,
        "status": 2, //2 编辑  4删除
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["newNumber"]
  }, [_c('div', {
    staticClass: ["inp"],
    on: {
      "click": function($event) {
        _vm.selectItem('选择收费类型')
      }
    }
  }, [_vm._m(0), _c('div', {
    staticClass: ["inp2"]
  }, [_c('text', {
    class: ['inp2_text', _vm.chooseItem == '请选择' ? '' : 'active']
  }, [_vm._v(_vm._s(_vm.chooseItem))])]), _c('div', {
    staticClass: ["inp3"]
  }, [_c('image', {
    staticClass: ["inp3_icon"],
    attrs: {
      "src": _vm.rightIcon
    }
  })])]), _c('div', {
    ...
    class: ['inpK_text2', _vm.inputText2 == '请输入车牌号' ? '' : 'inpK_textActive']
