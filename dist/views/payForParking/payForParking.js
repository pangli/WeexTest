...
/* styles */
__vue_styles__.push(__webpack_require__(635)
)

/* script */
__vue_exports__ = __webpack_require__(636)
...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/payForParking/payForParking.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-8704b8a0"
...
module.exports = {
  ...
  "header": {
    "width": "750",
    "height": "680",
    "backgroundImage": "linear-gradient(270deg,#06C9FE,#5865FD)"
  },
  "header-iOS7": {
    "paddingTop": "40",
    "height": "720"
...
var _base64ParkingPay = __webpack_require__(99);
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            rightIcon: (0, _base64ParkingPay.rightIcon)(),
            dateList: ['1个月', '2个月', '3个月', '4个月', '5个月', '6个月', '7个月', '8个月', '9个月', '10个月', '11个月', '12个月'],
            chooseDate: "1个月",
            chooseDateId: "1",
            dateListPark: "",
            dateListParkId: "",
    ...
    created: function created() {
        ...
        self.getParkList();
        // 监听编辑成功或者新增成功
        var paySuccess = new BroadcastChannel("paySuccess");
        paySuccess.onmessage = function (event) {
            if (event.data == '1') {
                self.getParkList();
            }
        };
        self.currentDayTime = new Date().getTime();
    },
    ...
    methods: {
        ...
        init: function init() {
            ...
            self.parkFeeDetails = "";
            // 充值到期时间
            self.getNextMonthDay = "";
            //充值缴费账单创建成功返回
            self.parkMonthlyBill = "";
            self.billingCycle = "";
            // 缴费到期时间时间
            self.endDay = "";
            // 下次缴费开始时间
        ...
        // 选择数据
        selectItem: function selectItem(title, dataList, type) {
            var self = this;
            ...
            picker.pick({
                ...
                if (res.result === 'success') {
                    if (type == '2') {
                        self.chooseDate = dataList[res.data];
                        self.chooseDateId = res.data + 1;
                        //充值到期时间
                        self.getNextMonthDay = _global2.default.getCrentDayNext(_global2.default.getNextMonthDay(self.nextStartDay, self.chooseDateId), -1);
                        self.needPayCount = self.parkFeeDetails.feeStandard * self.chooseDateId;
                    } else if (type == '1') {
                        self.showParkName = dataList[res.data];
                        self.showParkId = self.dateListParkId[res.data];
                        self.init(); //重置数据
                        self.myBindPlateNoDetail();
                    }
                } else {}
            });
        },
        goToHistory: function goToHistory() {
            var self = this;
            var url = url = _global2.default.getUrl(weex.config.bundleUrl, 'views/payForParking/payForParkingHistory.js' + "?parkId=" + self.showParkId);
            navigator.push({
                url: url,
                animated: 'true'
            });
        ...
        goToPay: function goToPay() {
            var self = this;
            var url = url = _global2.default.getUrl(weex.config.bundleUrl, 'views/payForParking/payCenter.js' + '?orderNo=' + self.parkMonthlyBill.orderNo + "&payAmount=" + self.parkMonthlyBill.amount + "&id=" + self.parkMonthlyBill.id);
            navigator.push({
                url: url,
                animated: 'true'
            });
        ...
        getParkList: function getParkList() {
            ...
            _api2.default.getParkList(params, function (ret) {
                ...
                if (!ret.success) {
                    _toast2.default.showErrorMessage(ret.message);
                    return;
                }
                if (ret.code == '10000' && ret.success) {
                    ...
                    _toast2.default.showMessage(ret.message);
                }
            });
        },
        myBindPlateNoDetail: function myBindPlateNoDetail() {
            //绑定的车位信息
            var self = this;
            var weexParams = weex.config.params || {};
            var params = {
                "mallId": weexParams.marketId,
                "parkId": self.showParkId,
                "userId": weexParams.userId
            };
            _api2.default.myBindPlateNoDetail(params, function (ret) {
                _toast2.default.close();
                // modal.alert({
                //   message:"ret"+JSON.stringify(ret)
                // })
                if (!ret.success) {
                    self.parkMonthlyBill = '1';
                    self.init(); //重置数据
                    _toast2.default.showErrorMessage(ret.message);
                    return;
                }
                if (ret.code == '10000' && ret.success) {
                    if (ret.body.BindPlateNO.status == 2) {
                        self.bindPlateNO = ret.body.BindPlateNO;
                        self.bindStatus = ret.body.bindStatus;
                        self.endDay = _global2.default.formatTime(ret.body.endDay, '-', 'yyyymmdd');
                        self.showEndDay = ret.body.endDay ? true : false;
                        self.startDay = ret.body.startDay;
                        self.nextStartDay = _global2.default.formatTime(ret.body.nextStartDay, '-', 'yyyymmdd');
                        self.billingCycle = ret.body.billingCycle;
                        //充值到期时间
                        self.getNextMonthDay = _global2.default.getCrentDayNext(_global2.default.getNextMonthDay(self.nextStartDay, 1), -1);
                        self.getOffsetDays = _global2.default.getOffsetDays(ret.body.endDay, Date.now());
                        self.sumRemainDay = ret.body.sumRemainDay;
                        self.getParkFeeDetail();
                    } else if (ret.body.BindPlateNO.status == 3) {
                        _toast2.default.showMessage('当前车位审核失败', 2);
                        self.init(); //重置数据
                    } else if (ret.body.BindPlateNO.status == 1) {
                        _toast2.default.showMessage('当前车位未审核', 2);
                        self.init(); //重置数据
                    } else if (ret.body.BindPlateNO.status == 4) {
                        _toast2.default.showMessage('当前车位已解绑', 2);
                        self.init(); //重置数据
                    }
                } else {
                    self.parkMonthlyBill = '1';
        ...
        getParkFeeDetail: function getParkFeeDetail() {
            //当前停车的收费类型
            var self = this;
            var weexParams = weex.config.params || {};
            var params = {
                "parkId": self.showParkId,
                "parkType": self.bindPlateNO.numberType == 2 ? 4 : 5 //1 临停计费标准 2 优惠临停计费标准 3 公司月租 4 个人月租 5、限时优惠停车分类 
            };
            _api2.default.getParkFeeDetail(params, function (ret) {
                _toast2.default.close();
                if (!ret.success) {
                    self.setParkPay = false;
                    self.parkFeeDetails = '';
                    self.parkMonthlyBill = '2';
                    _toast2.default.showErrorMessage(ret.message);
                    return;
                }
                if (ret.code == '10000' && ret.success) {
                    self.parkFeeDetails = ret.body.parkFee;
                    //
                    self.needPayCount = self.parkFeeDetails.feeStandard * self.chooseDateId;
                } else {
                    self.parkMonthlyBill = '2';
        ...
        addMonthlyBill: function addMonthlyBill() {
            //充值缴费
            var self = this;
            var weexParams = weex.config.params || {};
            if (self.parkMonthlyBill == '1') {
                modal.alert({
                    message: "无车位信息"
                });
                return;
            } else if (self.parkMonthlyBill == '2') {
                modal.alert({
                    message: "当前未配置收费类型"
                });
                return;
            ...
            var params = {
                "parkBindId": self.bindPlateNO.id,
                "amount": self.needPayCount,
                "monthCount": self.chooseDateId,
                "billingCycle": self.billingCycle,
                "startDay": self.nextStartDay + " 00:00:00",
                "endDay": self.getNextMonthDay + " 23:59:59",
                "numberType": self.bindPlateNO.numberType,
                "plateNo": self.bindPlateNO.plateNo,
                "userId": weexParams.userId
            };
            _api2.default.addMonthlyBill(params, function (ret) {
                _toast2.default.close();
                if (!ret.success) {
                    _toast2.default.showErrorMessage(ret.message);
                    return;
                ...
                if (ret.code == '10000' && ret.success) {
                    self.parkMonthlyBill = ret.body.ParkMonthlyBill;
                    // 跳转页面
                    self.goToPay();
                } else {
                    _toast2.default.showMessage(ret.message);
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.bindPlateNO.numberType == 2 ? '月租缴费' : '套餐缴费'))]), _c('div', {
  ...
  }, [_vm._v("缴费记录")])])]), (!_vm.bindPlateNO.numberType) ? _c('div', {
    staticClass: ["topList"]
  }, [_vm._m(0)]) : _vm._e(), (_vm.bindPlateNO.numberType == 2) ? _c('div', {
    staticClass: ["topList"]
  }, [_c('div', {
    staticClass: ["topItem"]
  }, [_c('text', {
    ...
    staticClass: ["item_count_c"]
  }, [_vm._v(_vm._s(_vm.getOffsetDays ? _vm.getOffsetDays : 0)), (_vm.startDay < _vm.currentDayTime) ? _c('text', {
    staticClass: ["countTips"]
  }, [_vm._v("下月生效")]) : _vm._e()])])])]) : _vm._e(), (_vm.bindPlateNO.numberType == 3) ? _c('div', {
    staticClass: ["topList"]
  }, [_c('div', {
    staticClass: ["topItem"]
  }, [_c('text', {
    staticClass: ["item_text"]
  }, [_vm._v("当月剩余天数")]), _c('text', {
    staticClass: ["item_count"]
  }, [_c('text', {
    staticClass: ["item_count_c"]
  }, [_vm._v(_vm._s(_vm.bindPlateNO.remainDay ? _vm.bindPlateNO.remainDay : 0)), (_vm.startDay < _vm.currentDayTime) ? _c('text', {
    staticClass: ["countTips"]
  }, [_vm._v("下月生效")]) : _vm._e()])])]), _c('div', {
    staticClass: ["topItem"]
  }, [_c('text', {
    staticClass: ["item_text"]
  }, [_vm._v("总剩余天数")]), _c('text', {
    staticClass: ["item_count"]
  }, [_c('text', {
    staticClass: ["item_count_c"]
  }, [_vm._v(_vm._s(_vm.sumRemainDay ? _vm.sumRemainDay : 0))])])])]) : _vm._e(), (_vm.showEndDay) ? _c('div', {
    staticClass: ["topdate"]
  }, [_c('text', {
    staticClass: ["topdate_text"]
  }, [_vm._v("到期日期：" + _vm._s(_vm.endDay))])]) : _vm._e(), _c('div', {
    staticClass: ["infoWrap"]
  }, [_c('div', {
    staticClass: ["info"]
  }, [_c('text', {
    staticClass: ["info_text"]
  }, [_vm._v(_vm._s(_vm.bindPlateNO.userName) + _vm._s(' ') + "|" + _vm._s(' ') + _vm._s(_vm.bindPlateNO.plateNo))]), _c('text', {
    staticClass: ["info_text2"]
  }, [_vm._v("租户名称：" + _vm._s(_vm.bindPlateNO.companyName))])])])]), _c('div', {
    class: ['contentlist', _vm.isiOS7 ? 'contentlist7' : '', _vm.isiPhoneX ? 'contentlistX' : '', _vm.showEndDay ? 'taoCan' : '', _vm.isiOS7 && _vm.showEndDay ? 'taoCan7' : '', _vm.isiPhoneX && _vm.showEndDay ? 'taoCanX' : '']
  }, [_c('list', {
    ...
    on: {
      "click": function($event) {
        _vm.selectItem('选择停车场', _vm.dateListPark, '1')
      }
    }
  }, [_c('text', {
    staticClass: ["lineItem_text"]
  }, [_vm._v("停车场")]), _c('text', {
    staticClass: ["lineItem_text2"]
  }, [_vm._v(_vm._s(_vm.showParkName))]), _c('image', {
    staticClass: ["lineItem_icon"],
    attrs: {
      "src": _vm.rightIcon
    }
  })]), _c('div', {
    ...
    on: {
      "click": function($event) {
        _vm.selectItem('选择日期', _vm.dateList, '2')
      }
    }
  }, [_c('text', {
    staticClass: ["lineItem_text"]
    ...
    staticClass: ["lineItem_text2"]
  }, [_vm._v(_vm._s(_vm.chooseDate))]), _c('image', {
    staticClass: ["lineItem_icon"],
    attrs: {
      "src": _vm.rightIcon
    }
    ...
    staticClass: ["lineItem_text2"]
  }, [_vm._v(_vm._s(_vm.nextStartDay))])]), _c('div', {
    staticClass: ["lineItem"]
  }, [_c('text', {
    staticClass: ["lineItem_text"]
  }, [_vm._v("到期日期")]), _c('text', {
    staticClass: ["lineItem_text2"]
  }, [_vm._v(_vm._s(_vm.getNextMonthDay))])]), _c('div', {
    staticClass: ["lineItem"]
  }, [_c('text', {
    staticClass: ["lineItem_text"]
  }, [_vm._v("月租金额")]), _c('text', {
    staticClass: ["lineItem_text2"]
  }, [_vm._v("￥" + _vm._s(_vm.parkFeeDetails.feeStandard))])]), _c('div', {
    staticClass: ["lineItem3"]
  }, [_c('text', {
    staticClass: ["lineItem_text"]
  }, [_vm._v("需支付金额")]), _c('text', {
    staticClass: ["lineItem_text2", "lineItem_text3"]
  }, [_vm._v("￥" + _vm._s(_vm.needPayCount))])])])])]), _c('div', {
    class: ['btnWrap', _vm.isiPhoneX ? 'btnWrapX' : '']
  }, [_c('div', {
    staticClass: ["btn"],
    on: {
      "click": _vm.addMonthlyBill
    }
  }, [_c('text', {
...
var car = function car() {
  return 'data:image/png;base64,';
};
