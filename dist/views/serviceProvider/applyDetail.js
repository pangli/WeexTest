...
/* styles */
__vue_styles__.push(__webpack_require__(667)
)
...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/serviceProvider/applyDetail.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-fa282c32"
...
var _projectProgress = __webpack_require__(669);
...
exports.default = {
  components: {
    ...
    projectProgress: _projectProgress2.default
  },
  data: {
    icon: _icon2.default,
    flag: true,
  ...
  created: function created() {
    ...
    var come = new BroadcastChannel('refresh');
    come.onmessage = function (event) {
      _this.getProviderDetailData();
    };
    this.getProviderDetailData();
  },
  ...
  methods: {
    ...
    setContent: function setContent(val) {
      if (val) {
        ...
        _toast2.default.showMessage("网址复制成功", 1);
      }
    },
    ...
    myApply: function myApply() {
      _global2.default.jump("views/serviceProvider/apply.js", "?id=" + this.providerId);
    },
    getProviderDetailData: function getProviderDetailData() {
      var that = this;
      that.programData = [{ text: '已申请', createTime: '', status: "1" }, { text: '已确认', createTime: '', status: "0" }, { text: '已联系', createTime: '', status: "0" }, { text: '合作成功', createTime: '', status: "0" }];
      _toast2.default.showLoadingMessage('加载中...');
      _api2.default.getProviderDetailData({ "detailId": this.providerId }, function (res) {
        console.log(res);
        _toast2.default.close();
        if (res.res.code == 10000) {
          that.detailData = res.body.output;
          that.programData[0].createTime = res.body.output.createTime; //已申请时间
          that.status = that.detailData.status;
          if (that.detailData.status == 4) {
            that.programData = [{ text: '已申请', createTime: that.detailData.createTime, status: "1" }, { text: '已确认', createTime: that.detailData.confirmTime, status: "1" }, { text: '已联系', createTime: that.detailData.contactTime, status: "1" }, { text: '合作成功', createTime: that.detailData.actionTime, status: "1" }];
          } else if (that.detailData.status == 3) {
            if (that.detailData.confirmTime) {
              that.programData[1].status = '1';
              that.programData[1].createTime = that.detailData.confirmTime;
              if (that.detailData.contactTime) {
                that.programData[2].status = '1';
                that.programData[2].createTime = that.detailData.contactTime;
                that.programData[3].status = '2';
                that.programData[3].text = '合作失败';
                that.programData[3].createTime = that.detailData.actionTime;
              } else {
                that.programData[2].status = '2';
                that.programData[2].text = '合作失败';
                that.programData[2].createTime = that.detailData.actionTime;
              }
            } else {
              that.programData[1].status = '2';
              that.programData[1].text = '合作失败';
              that.programData[1].createTime = that.detailData.actionTime;
            }
          } else if (that.detailData.status == 2) {
            that.programData[1].status = '1';
            that.programData[2].status = '1';
            that.programData[1].createTime = that.detailData.confirmTime;
            that.programData[2].createTime = that.detailData.contactTime;
          } else if (that.detailData.status == 1) {
            that.programData[1].status = '1';
            that.programData[1].createTime = that.detailData.confirmTime;
          }
        }
    ...
    toEvaluate: function toEvaluate(type) {
      if (type == 1) {
        _global2.default.jump("views/evaluate/viewEvaluation.js", "?come=14&marketId=" + this.detailData.marketId + '&id=' + this.detailData.id);
      } else {
        _global2.default.jump("views/evaluate/evaluate.js", "?come=14&referenceName=" + encodeURI(this.detailData.providerName) + "&marketId=" + this.detailData.marketId + '&id=' + this.detailData.id + '&referenceNo=' + this.detailData.detailNo);
      }
    },
    Reapply: function Reapply() {
      _global2.default.jump("views/serviceProvider/apply.js", "?id=" + this.detailData.providerId);
    }
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [(_vm.status == 3 || _vm.status == 4) ? _c('div', {
    staticClass: ["apply_status"]
  }, [(_vm.status == 3) ? _c('text', {
    staticClass: ["fail_text"]
  }, [_vm._v("未达成合作，期待下次合作！")]) : _vm._e(), (_vm.status == 4) ? _c('text', {
    staticClass: ["succ_text"]
  }, [_vm._v("合作成功，感谢支持！")]) : _vm._e()]) : _vm._e(), _c('projectProgress', {
    attrs: {
      "list": _vm.programData
    }
  }), _c('div', {
    staticClass: ["modal"]
  }, [_c('div', {
    staticClass: ["service_mess"]
  }, [_c('image', {
    staticClass: ["mess_left"],
    attrs: {
      "resize": "cover",
      "src": _vm.detailData.providerImgUrl
    }
  }), _c('div', {
    staticClass: ["mess_right"]
  }, [_c('div', {
    staticClass: ["mess_title"]
  }, [_c('text', {
    staticClass: ["store_title"],
    attrs: {
      "lines": "2"
    }
  }, [_vm._v(_vm._s(_vm.detailData.providerName))])])])]), _c('div', {
    staticClass: ["basic_mess"]
  }, [_c('text', {
    staticClass: ["common_title"]
  }, [_vm._v("基本信息")]), _c('div', {
    staticClass: ["list_mess"]
  }, [_c('text', {
    staticClass: ["text_left"]
  }, [_vm._v("所属行业:")]), _c('text', {
    staticClass: ["text_right"]
  }, [_vm._v(_vm._s(_vm.detailData.providerTypeName))])]), _c('div', {
    staticClass: ["list_mess"],
    on: {
      "longpress": function($event) {
        _vm.setContent(_vm.detailData.providerWebSite)
      }
    }
  }, [_c('text', {
    staticClass: ["text_left"]
    ...
    staticClass: ["text_right"]
  }, [_vm._v(_vm._s(_vm.detailData.providerWebSite))])]), _c('div', {
    staticClass: ["list_mess"]
  }, [_c('text', {
    staticClass: ["text_left"]
  }, [_vm._v("联系电话:")]), _c('text', {
    ...
    on: {
      "click": function($event) {
        _vm.call(_vm.detailData.providerPhone)
      }
    }
  }, [_vm._v(_vm._s(_vm.detailData.providerPhone))])])])]), _c('div', {
    staticClass: ["modal"]
  }, [_c('div', {
    staticClass: ["basic_mess"]
  }, [_c('text', {
    ...
    staticClass: ["text_right"]
  }, [_vm._v(_vm._s(_vm.detailData.enterprise))])]), _c('div', {
    staticClass: ["list_mess"]
  }, [_c('text', {
    staticClass: ["text_left"]
  }, [_vm._v("合作诉求")]), _c('text', {
    staticClass: ["text_right"]
  }, [_vm._v(_vm._s(_vm.detailData.memo))])])])])], 1)]), (_vm.status == 3 || _vm.status == 4) ? _c('div', {
    class: ['footer', _vm.isiPhoneX ? 'footerx' : '']
  }, [(_vm.status == 3) ? _c('text', {
    staticClass: ["com_btn", "again"],
    on: {
      "click": _vm.Reapply
    }
  }, [_vm._v("重新申请")]) : _vm._e(), (_vm.detailData.hasTemp == 1) ? _c('text', {
    staticClass: ["com_btn", "evalute"],
    on: {
      "click": function($event) {
        _vm.toEvaluate(_vm.detailData.isEvaluate)
      }
    }
  }, [_vm._v(_vm._s(_vm.detailData.isEvaluate == 0 ? '立即评价' : '查看评价'))]) : _vm._e()]) : _vm._e()], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
