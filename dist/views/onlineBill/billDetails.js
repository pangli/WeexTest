...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/onlineBill/billDetails.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-2489c636"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports = {
  ...
  "header": {
    "width": "750",
    "height": "440",
    "backgroundImage": "linear-gradient(to bottom,rgb(0,144,255),rgb(0,189,255))"
  },
  "header-iOS7": {
    "paddingTop": "40",
    "height": "480"
  ...
  "details": {
    ...
    "paddingLeft": 0,
    "borderBottomWidth": "1",
    "borderBottomColor": "rgb(52,196,255)"
  },
  "total": {
    "height": "93",
    "paddingTop": 0,
  ...
  "toPay": {
    ...
    "height": "76",
    "borderRadius": "38",
    "backgroundImage": "linear-gradient(to right,rgba(0,154,255,1),rgba(0,189,255,1))",
    "position": "fixed",
    "left": "60",
    "bottom": "16"
  },
...
exports.default = {
  ...
  filters: {
    iconMap: function iconMap(val) {
      ...
      //   '其它': otherIcon()
      // }
      var icon = (0, _base.otherIcon)();
      if (!val) ;else if (val.indexOf('电费') >= 0) icon = (0, _base.electricIcon)();else if (val.indexOf('租金') >= 0) icon = (0, _base.rentIcon)();else if (val.indexOf('水费') >= 0) icon = (0, _base.waterIcon)();

      return icon;
    },
    formatTime: function formatTime(val) {
      return val ? _global2.default.formatTime(new Date(val), '.', 'yyyymmdd') : '';
      // return val;
    }
  },
  data: function data() {
    return {
      ...
      electricIcon: (0, _base.electricIcon)(),
      costAmount: 0.00,
      billInfo: {},
      billList: [],
      billInfo0: {
      ...
      billList0: [{
        "arrearsBill": 28640.33, //待缴金额
        "reductionAmount": 0.0, //减免金额
        "endPayDate": 1527696000000,
        "amountPaid": 0.0, //已缴金额
        "endDate": 1530288000000,
        "actualAmount": 28640.33, //应缴金额
        "typeName": "固定租金",
        "statusName": "未缴",
        "startDate": 1527782400000
      }, {
        "arrearsBill": 27641.33,
        "reductionAmount": 0.0,
        "endPayDate": 1527696000000,
        "amountPaid": 0.0,
        "endDate": 1527696000000,
        "actualAmount": 28640.33,
        "typeName": "固定租金",
        "statusName": "未缴",
        "startDate": 1525104000000
      }],
      result: '',
      orderNo: "",
      bodyUrl: ""
  ...
  created: function created() {
    ...
    var self = this;
    if (WXEnvironment.platform == "Web") {
      this.billInfo = this.billInfo0;
      this.billList = this.billList0;
    }
  ...
  methods: {
    ...
    goPage: function goPage(id) {
      var url = url = _global2.default.getUrl(weex.config.bundleUrl, './noticeDetails.js');
      navigator.push({
        url: url,
        animated: 'true'
      });
    },
    getBillDetails: function getBillDetails() {
      var _this2 = this;
      ...
      var params = {
        ...
        "id": param.id || weexParams.id,
        "marketId": weexParams.marketId
      };

      this.result = params;
      _toast2.default.showLoadingMessage("加载中…");
      _api2.default.geBillingDetailByParam(params, function (ret) {
        _toast2.default.close();
        ...
        //  modal.alert({
        //    message:"detailsRet"+JSON.stringify(ret)
        // })

        if (ret.res.code == '10000' && ret.body) {
          var data = ret.body;
          if (data) {
            _this2.billInfo = data.billInfo;
            _this2.billList = data.billList;
            _this2.orderNo = data.billInfo.orderNo;
            //提取最后缴款日期
            _this2.billInfo.endPayDate = data.billList[0].endPayDate;

            //计划待缴账单总额
            var costAmount = 0;
            _this2.billList.forEach(function (item) {
              costAmount += item.arrearsBill;
            });
            _this2.costAmount = costAmount.toFixed(2);

            if (_this2.billList.length == 0) _toast2.default.showErrorMessage('暂无数据');
          }
    ...
    payBill: function payBill() {
      ...
      var weexParams = weex.config.params || {};
      var params = {
        "arrearsNo": self.billInfo.arrearsNo,
        "reductionAmount": 0,
        "actualAmount": self.costAmount,
        "costAmount": 0,
        "amountPaid": 0,
        "contractNo": self.billInfo.contractNo,
        "accountId": 0,
        "marketId": weexParams.marketId,
        "phone": weexParams.phone,
        "createUser": weexParams.userId,
        'createName': weexParams.userTrueName
      };

      _toast2.default.showLoadingMessage("加载中…");
      // modal.alert({
      //   message:"weexParams"+JSON.stringify(weexParams)
      // })
      // modal.alert({
      //   message:"params"+JSON.stringify(params)
      // })
      _api2.default.insertAccountOrder(params, function (ret) {
        _toast2.default.close();
        // this.result = ret;
        if (!ret.res) {
          _toast2.default.showErrorMessage('服务异常');
        ...
        if (ret.res.code == '10000' && ret.body) {
          self.orderNo = ret.body.orderNo;
          // self.getActivePayOrder(ret.body.orderNo)
          self.billPayPage(ret.body.orderNo);
        } else {
          _toast2.default.showMessage(ret.res.msg);
        }
      });
    },
    billPayPage: function billPayPage(orderNo) {
      var url = url = _global2.default.getUrl(weex.config.bundleUrl, 'views/onlineBill/billPayPage.js?orderNo=' + orderNo);
      navigator.push({
        url: url,
        animated: 'true'
      });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("账单详情")]), _vm._m(0)]), _c('div', {
    staticClass: ["details"]
  }, [_c('div', {
    staticClass: ["flex-dir-row"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-30"]
  }, [_vm._v("商户名称")]), _c('text', {
    staticClass: ["t-white", "f-size-30", "m-left-20"]
  }, [_vm._v(_vm._s(_vm.billInfo.merchantName))])]), _c('div', {
    staticClass: ["flex-dir-row", "m-top-30"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-30"]
  }, [_vm._v("合同名称")]), _c('text', {
    staticClass: ["t-white", "f-size-30", "m-left-20", "flex-1"]
  }, [_vm._v(_vm._s(_vm.billInfo.contractNo))])]), _c('div', {
    staticClass: ["flex-dir-row", "m-tb-30"]
  }, [_c('div', {
    staticClass: ["flex-1", "flex-dir-row"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-24"]
  }, [_vm._v("所属项目")]), _c('text', {
    staticClass: ["t-white", "f-size-24", "m-left-20"]
  }, [_vm._v(_vm._s(_vm.billInfo.project))])]), _c('div', {
    staticClass: ["flex-1", "flex-dir-row"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-24"]
  }, [_vm._v("账单编号")]), _c('text', {
    staticClass: ["t-white", "f-size-24", "m-left-20"]
  }, [_vm._v(_vm._s(_vm.billInfo.arrearsNo))])])]), _c('div', {
    staticClass: ["flex-dir-row"]
  }, [_c('div', {
    staticClass: ["flex-1", "flex-dir-row"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-24"]
  }, [_vm._v("最后缴款日")]), _c('text', {
    staticClass: ["t-white", "f-size-24", "m-left-20"]
  }, [_vm._v(_vm._s(_vm._f("formatTime")(_vm.billInfo.endPayDate)))])]), _c('div', {
    staticClass: ["flex-1", "flex-dir-row"]
  })])]), _c('div', {
    staticClass: ["total"]
  }, [_c('text', {
    staticClass: ["t-gray", "f-size-30"]
  }, [_vm._v("账单待缴金额")]), _c('text', {
    staticClass: ["t-white", "f-size-40"]
  }, [_vm._v(_vm._s(_vm.costAmount) + "元")])])]), _c('list', {
    staticClass: ["list"]
  }, [_c('downLoading', {
    ref: "refresh",
    on: {
    ...
    return (_vm.billList.length > 0) ? _c('cell', {
      staticClass: ["cell"],
      appendAsTree: true,
      attrs: {
        "append": "tree"
      ...
      staticClass: ["bill-item", "flex-dir-row"]
    }, [_c('image', {
      staticClass: ["item-icon"],
      attrs: {
        "resize": "contain",
        "src": _vm._f("iconMap")(item.typeName)
      }
    }), _c('text', {
      staticClass: ["f-size-30", "t-color-999", "m-left-10"]
    }, [_vm._v(_vm._s(item.typeName))]), _c('div', {
      staticClass: ["line"]
    }), _c('text', {
      staticClass: ["f-size-30", "t-color-999"]
    }, [_vm._v(_vm._s(item.statusName))]), _c('text', {
      staticClass: ["f-size-32", "t-color-333", "arrears-money"]
    }, [_vm._v(_vm._s(item.arrearsBill.toFixed(2)) + "元")])]), _c('div', {
      staticClass: ["bill-detail"]
    }, [_c('div', {
      staticClass: ["flex-dir-row"]
    }, [_c('div', {
      staticClass: ["flex-dir-row", "flex-1"]
    }, [_c('text', {
      staticClass: ["f-size-26", "t-color-999"]
    }, [_vm._v("应缴金额")]), _c('text', {
      staticClass: ["f-size-26", "t-color-333", "m-left-22"]
      ...
      staticClass: ["flex-dir-row", "m-top-27"]
    }, [_c('div', {
      staticClass: ["flex-dir-row", "flex-1"]
    }, [_c('text', {
      staticClass: ["f-size-26", "t-color-999"]
      ...
      staticClass: ["f-size-26", "t-color-333", "m-left-22"]
    }, [_vm._v(_vm._s(item.reductionAmount.toFixed(2)) + "元")])]), _c('div', {
      staticClass: ["flex-dir-row", "flex-1"]
    }, [_c('text', {
      staticClass: ["f-size-26", "t-color-999"]
    }, [_vm._v("待缴金额")]), _c('text', {
      ...
      staticClass: ["f-size-26", "t-color-333", "m-left-22"]
    }, [_vm._v(_vm._s(_vm._f("formatTime")(item.startDate)) + "-" + _vm._s(_vm._f("formatTime")(item.endDate)))])])])]) : _vm._e()
  })], 2), (_vm.billInfo.costAmount > 0 && _vm.billInfo.payStatus == '1') ? _c('div', {
    class: ['toPay', _vm.isiPhoneX ? 'toPayX' : ''],
    on: {
      "click": _vm.payBill
    }
  }, [_c('text', {
    staticClass: ["toPay_text"]
  }, [_vm._v("立即支付")])]) : _vm._e()])
