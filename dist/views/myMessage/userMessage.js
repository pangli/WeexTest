...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/myMessage/userMessage.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4d447179"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  filters: {
    phoneSymbol: function phoneSymbol(value) {
      if (!value) return '';
      return value.substr(0, 3) + '****' + value.substr(7);
    }
  },
  watch: {
    nickname: function nickname(newValue) {
  ...
  mounted: function mounted() {
    ...
    var img = 'my-avatar.png';
    if (WXEnvironment.osName == 'iOS') {
      img = '../images/' + img;
    } else if (WXEnvironment.osName == 'android') {
      img = 'file:///android_asset/dist/views/images/' + img;
    } else {
      img = '../../../images/' + img;
    }
    ...
    var sexMap = { '1': '男', '2': '女' };
    var weexParams = weex.config.params || {};
    this.photo = weexParams.photoPath || img;
    this.nickname = weexParams.userName;
    this.username = weexParams.userTrueName;
    this.sex = sexMap[weexParams.gender];
    this.birthday = weexParams.birthday;
    this.phoneNumber = weexParams.phone;
    this.companyName = weexParams.company;
    this.departmentName = weexParams.department;
    this.email = weexParams.email;
    this.result = weexParams;

    //接收手机号更改刷新
    var broadcast = new BroadcastChannel('userInforUpdate');
    broadcast.onmessage = function (event) {
      console.log(event.data);
      if (event.data) _this2.phoneNumber = event.data;
    };
  ...
  methods: {
    ...
    pickSex: function pickSex() {
      var _this = this;
      if (normalPicker) {
        var params = { "options": ["男", "女"] };
      ...
      picker.pick({
        ...
        if (ret.result == 'success') {
          _this.sex = _this.sexArr[ret.data];
        }
      });
    },
    pickBirthday: function pickBirthday() {
      var _this = this;
      ...
      if (timePicker) {
        var params = {
          ...
          "beginMonth": 0,
          "beginDay": 1,
          "endYear": objDate.getFullYear(),
          "endMonth": objDate.getMonth() + 1,
          "endDay": objDate.getDate()
        };
        if (this.birthday) {
          var arr = this.birthday.split('-');
          if (arr.length > 1) {
            params.setYear = arr[0];
            params.setMonth = arr[1];
            params.setDay = arr[2];
          }
        }
        this.result = params;
      ...
      picker.pickDate({
        ...
        if (ret.result == 'success') {
          _this.birthday = ret.data;
        }
      });
    },
    actionSheetOpen: function actionSheetOpen() {
      this.$refs.nickname.blur();
      if (this.$refs.childMethod) this.$refs.childMethod.showPage(true);else toast.showErrorMessage('$refs.childMethod.showPage 未定义!');
    },
    selected: function selected(val) {
      this.result = val;
      if (val && val.src && val.src.length > 0) {
        this.photo = val.src[0];
        this.postResult = this.photo;
        this.photoNew = 1;
      }
      /*if(val.index==0){
        general.openImagePicker(1,(obj)=>{
          this.photo = obj.photos[0];
          this.postResult = this.photo;
          this.photoNew = 1;
        });
      }else if(val.index==1){
        general.takePicture((obj)=>{
          this.photo = obj.photos[0];
          this.postResult = this.photo;
          this.photoNew = 1;
        });
    ...
    jumpChangePhone: function jumpChangePhone() {
      this.$refs.nickname.blur();
      var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/myMessage/changePhone.js');
      navigator.push({
        url: url,
        animated: 'true'
      });
    ...
    uploadImage: function uploadImage() {
      ...
      this.result = params;
      toast.showLoadingMessage("图片上传中…");
      (0, _uploadService2.default)(params, function (ret) {
        _this3.result = ret;
        if (ret.res && ret.returnJson && ret.returnJson.res && ret.returnJson.res.code == 10000 && ret.returnJson.body) {
          _this3.imgId = ret.returnJson.body.imgId;
          _this3.photoNew = 0;
          _this3.imgUrl = ret.returnJson.body.imgUrl;
          toast.close();
          _this3.updateUserInfo();
        } else {
          toast.showErrorMessage('头像上传失败!');
        }
      });
    },
    ...
    updateUserInfo: function updateUserInfo() {
      ...
      if (!this.nickname) {
        toast.showMessage('请填写昵称!');
        return;
      } else if (/[^\w\u4e00-\u9fa5]/g.test(this.nickname)) {
        toast.showMessage('昵称不能有特殊字符!');
        return;
      }
      ...
      var params = {
        'accessToken': weexParams.accessToken,
        'userId': weexParams.userId
        // 'gender': mapSex[this.sex],
        // 'birthday': this.birthday,
        // 'userName': this.nickname,
        // 'photoPath': this.imgId
      };
      if (this.sex && mapSex[this.sex]) params.gender = mapSex[this.sex];
      if (this.birthday) params.birthday = this.birthday;
      if (this.nickname) params.userName = this.nickname;
      if (this.imgId) params.photoPath = this.imgId;
      if (this.email) params.email = this.email;

      //        this.result = params;

      toast.showLoadingMessage("请稍候…");
      _api2.default.updateUserInfo(params, function (ret) {
        toast.close();
        if (!ret.res) {
        ...
        if (ret.res.code == '10000') {
          //let data = ret.body;
          toast.showSuccessMessage(ret.res.msg);
          if (_this4.imgUrl) params.photoPath = _this4.imgUrl;
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["list_val"],
    on: {
      "click": _vm.pickBirthday
    }
  }, [_vm._v(_vm._s(_vm.birthday))]), _c('image', {
    staticClass: ["list_arror"],
    staticStyle: {
      width: "11px",
      height: "20px"
    ...
    staticClass: ["list_val"],
    on: {
      "click": _vm.jumpChangePhone
    }
  }, [_vm._v(_vm._s(_vm._f("phoneSymbol")(_vm.phoneNumber)))]), _c('image', {
    staticClass: ["list_arror"],
    staticStyle: {
      width: "11px",
      height: "20px"
    ...
    on: {
      "input": function($event) {
        _vm.companyName = $event.target.attr.value
      }
    }
  })]), _c('div', {
    staticClass: ["list_box"]
    ...
    on: {
      "input": function($event) {
        _vm.departmentName = $event.target.attr.value
      }
    }
  })])])])]), _c('div', {
