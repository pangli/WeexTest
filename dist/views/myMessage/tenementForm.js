...
__vue_options__.__file = "/Users/denya/Desktop/workspace/weex/h5/src/views/myMessage/tenementForm.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-16422b86"
...
exports.default = {
  ...
  mounted: function mounted() {
    ...
    this.phone = weexParams.tenantPhone || weexParams.phone;

    if (weexParams.userTrueName) {
      this.userName = weexParams.userTrueName;
      this.email = weexParams.email;
    }

    var Stack = new BroadcastChannel('getCompanyList');
    Stack.onmessage = function (event) {
      _this.company = event.data;
      if (event.data) {
        var arr = event.data.split(',');
        _this.company = arr[0];
        _this.companyId = arr[1];
      }
    };
  ...
  methods: {
    goBack: function goBack() {
      if (_global2.default.getParams(weex.config.bundleUrl).goBack == 2) {
        navigator.pop({
          closeLevel: 2
        });
        // navigator.pop();
    ...
    selectCompany: function selectCompany() {
      var url = _global2.default.getUrl(weex.config.bundleUrl, 'views/myMessage/companyList.js?id=' + this.companyId);
      navigator.push({
        url: url,
        animated: 'true'
      });
    ...
    confirmApprove: function confirmApprove() {
      ...
      if (!this.userName) {
        toast.showMessage('请输入用户名');
        return;
      } else if (/[^\w\u4e00-\u9fa5]/g.test(this.userName)) {
        toast.showMessage('用户名不能有特殊字符');
        return;
      }
      ...
      if (!this.email) {
        toast.showMessage('请输入邮箱地址');
        return;
      } else if (!reg.test(this.email.toLowerCase())) {
        //正则验证不通过，格式不对
        toast.showMessage('请输入正确邮箱地址');
        return;
      }

      if (!this.company) {
        toast.showMessage('请输入公司名');
        return;
      }
      ...
      toast.showLoadingMessage("请稍候…");
      _api2.default.userTenant(params, function (ret) {
        if (!ret.res) {
        ...
        if (ret.res.code == '10000') {
          toast.showSuccessMessage("认证提交成功!");
          _BroadcastChannel2.default.tenementSend('refresh');
          setTimeout(function () {
            navigator.pop();
          }, 1000);
        } else {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._m(0), (_vm.weexParams.userTrueName) ? _c('text', {
    staticClass: ["list_val"]
  }, [_vm._v(_vm._s(_vm.weexParams.userTrueName))]) : _c('input', {
    staticClass: ["list_val"],
    attrs: {
      "maxlength": "20",
      "type": "text",
  ...
  }), (!_vm.weexParams.userTrueName) ? _c('image', {
    ...
    staticClass: ["list_box"],
    on: {
      "click": _vm.selectCompany
    }
  }, [_vm._m(2), _c('text', {
    staticClass: ["list_val"]
  }, [_vm._v(_vm._s(_vm.company))]), _c('image', {
    staticClass: ["list_arror"],
    staticStyle: {
      width: "11px",
      height: "20px"
    ...
    on: {
      "input": function($event) {
        _vm.department = $event.target.attr.value
      }
    }
  }), _c('image', {
    staticClass: ["list_arror"],
